...
const SEQUENCE = [
  ...
  { id: "bhill_d", ti: 4,  ro: 4,  start: T_BACKHILL,  dur: SLIDE_DUR, anim: "slideUp" },
  { id: "fhill_d", ti: 12, ro: 7,  start: T_FC,         dur: SLIDE_DUR, anim: "slideUp" },
  ...
  { id: "fhill_l", ti: 13, ro: 8,  start: T_LITE,       dur: DUR_LITE,  anim: "fadeIn"  },
  { id: "cast_l",  ti: 15, ro: 10, start: T_LITE,       dur: DUR_LITE,  anim: "fadeIn"  },
  { id: "title",   ti: 19, ro: 12, start: T_TITLE,      dur: DUR_TITLE, anim: "slideUp" },
];
...
const JERK_DUR = 0.10; // seconds per jerk slide
...
const STORM_CLOUD_DEFS = [
  ...
  { id: "dc0", ti: 6, ro: 2, yFac: 0.43,
    jerks: [[0.0, -0.20], [1.3, -0.10], [2.5, -0.23], [3.8, -0.12], [5.1, -0.20]] },
  { id: "dc1", ti: 7, ro: 2, yFac: 0.47,
    jerks: [[0.0,  0.22], [1.5,  0.11], [2.8,  0.25], [4.1,  0.14], [5.4,  0.22]] },
  { id: "dc2", ti: 6, ro: 2, yFac: 0.39,
    jerks: [[0.0, -0.04], [1.1, -0.15], [2.3, -0.02], [3.5, -0.13], [4.8, -0.05]] },
  { id: "dc3", ti: 7, ro: 2, yFac: 0.41,
    jerks: [[0.0,  0.36], [1.4,  0.26], [2.7,  0.38], [4.0,  0.28], [5.3,  0.36]] },
  // ── Gray overlay — lighter beat, ~0.8–1.0 s between jerks, different phase ─
  { id: "gc0", ti: 8, ro: 3, yFac: 0.30,
    jerks: [[0.0,  0.12], [0.9,  0.22], [1.8,  0.08], [2.7,  0.19], [3.6,  0.07], [4.5,  0.17]] },
  { id: "gc1", ti: 9, ro: 3, yFac: 0.34,
    jerks: [[0.0, -0.28], [0.7, -0.16], [1.6, -0.30], [2.5, -0.18], [3.4, -0.26], [4.3, -0.14]] },
  { id: "gc2", ti: 8, ro: 3, yFac: 0.26,
    jerks: [[0.0,  0.28], [1.0,  0.16], [2.0,  0.30], [3.0,  0.18], [4.0,  0.27]] },
  { id: "gc3", ti: 9, ro: 3, yFac: 0.37,
    jerks: [[0.0, -0.10], [0.8, -0.21], [1.7, -0.06], [2.6, -0.19], [3.5, -0.08], [4.4, -0.20]] },
];
...
const LITE_CLOUD_DEFS = [
  { id: "wc0", ti: 10, darkTi: 8, ro: 6, yFac: 0.42, startT: T_WCLOUD1, loop: true,
    jerks: [[T_WCLOUD1, 0.06], [T_WCLOUD1 + 2.8, 0.02], [T_WCLOUD1 + 5.5, 0.07], [T_WCLOUD1 + 8.2, 0.06]] },
  { id: "wc1", ti: 11, darkTi: 9, ro: 6, yFac: 0.34, startT: T_WCLOUD2, loop: true,
    jerks: [[T_WCLOUD2, -0.07], [T_WCLOUD2 + 3.1, -0.03], [T_WCLOUD2 + 6.0, -0.08], [T_WCLOUD2 + 9.0, -0.07]] },
];
...
function SceneContent({
  ...
  useFrame(({ clock }) => {
    ...
    if (t >= T_LITE) {
      for (const id of DARK_LAYER_IDS) {
        const mesh = refs.current[id];
        if (!mesh) continue;
        mesh.material.opacity = 1 - easeOut(Math.min(1, (t - T_LITE) / DUR_LITE));
      }
    }
    ...
    for (const { id, jerks, yFac } of STORM_CLOUD_DEFS) {
      ...
      if (t < 0.8) {
        ...
        mesh.material.opacity = Math.max(0, 1 - easeOut((t - T_LITE) / DUR_LITE));
      } else {
    ...
    for (const { id, jerks, yFac, startT: st, loop } of LITE_CLOUD_DEFS) {
      ...
      if (darkMesh) {
        darkMesh.position.x = jerkX(Math.max(st, t - LITE_FOLLOW_DELAY), jerks, W, loop);
    ...
    if (menuFiredT.current !== null) {
      const p = easeOut(Math.min(1, (t - menuFiredT.current) / CASTLE_SLIDE_DUR));
  ...
  return (
    <>
      ...
      {LITE_CLOUD_DEFS.map(({ id, darkTi, ro }) => fullMesh(id + "_dark", darkTi, ro))}
