...
const ALL_TEXTURE_PATHS = [
  ...
  T("title_lite"),                 // 19
  T("flower_sprite"),              // 20
  T("CD1"), T("CD2"), T("CD3"), T("CD4"), T("CD5"), // 21–25
  T("CD6"), T("CD7"), T("CD8"), T("CD9"),            // 26–29
];
...
const S1_BOLT_ON  = 0.00;
const S1_BOLT_OFF = 0.20;
const S1_FB_ON    = 0.35;  // 150ms dark pause after bolt
const S1_FG_ON    = 0.50;  // fg overlays the last 100ms of the bg flash
const S1_OFF      = 0.60;  // everything off

// 500ms breathing space between strikes
// Strike 2: same pattern
const S2_BOLT_ON  = 1.10;
const S2_BOLT_OFF = 1.30;
const S2_FB_ON    = 1.45;
const S2_FG_ON    = 1.60;
const S2_OFF      = 1.70;
...
const SEQUENCE = [
  ...
  { id: "bhill_d", ti: 4,  ro: 2, start: T_BACKHILL, dur: SLIDE_DUR, anim: "slideUp" },
  // Front hill + castle arrive at the same time
  { id: "fhill_d", ti: 12, ro: 5, start: T_FC,   dur: SLIDE_DUR, anim: "slideUp"    },
  ...
  { id: "fhill_l", ti: 13, ro: 6, start: T_LITE, dur: DUR_LITE,  anim: "fadeIn"     },
  { id: "cast_l",  ti: 15, ro: 8, start: T_LITE, dur: DUR_LITE,  anim: "fadeIn"     },

  // White clouds — drop from above with elastic bounce, then drift
  { id: "cloud1",  ti: 10, ro: 4, start: T_CLOUD1, dur: DUR_CLOUD, anim: "bounceCloud", driftPhase: 0 },
  { id: "cloud2",  ti: 11, ro: 4, start: T_CLOUD2, dur: DUR_CLOUD, anim: "bounceCloud", driftPhase: Math.PI * 0.7 },

  // Title (lite version — we're in day mode by this point)
  { id: "title",   ti: 19, ro: 9, start: T_TITLE, dur: DUR_TITLE, anim: "slideUp"   },
];

// ─── Inner scene ──────────────────────────────────────────────────────────────

function SceneContent({ textures, onMenuReady, playLightningStrike, playThunderStrike, playMusic, playBirds }) {
  ...
  useFrame(({ clock }) => {
      ...
      switch (anim) {
        ...
        case "bounceCloud": {
          ...
          if (raw < 1) {
            ...
            mesh.position.y = restY + Math.sin(t * omega + (driftPhase || 0)) * amp;
          }
          break;
        }
      }
    ...
    const boltOn = (t >= T_FLASH + S1_BOLT_ON && t < T_FLASH + S1_BOLT_OFF) ||
                   (t >= T_FLASH + S2_BOLT_ON && t < T_FLASH + S2_BOLT_OFF);
    const fbOn   = (t >= T_FLASH + S1_FB_ON  && t < T_FLASH + S1_OFF) ||
                   (t >= T_FLASH + S2_FB_ON  && t < T_FLASH + S2_OFF);
    const ffOn   = (t >= T_FLASH + S1_FG_ON  && t < T_FLASH + S1_OFF) ||
  ...
  const mesh = (id, ti, ro) => (
    <mesh key={id} renderOrder={ro} ref={(m) => { if (m) refs.current[id] = m; }}>
...
function PreloadedScene({ onMenuReady }) {
  ...
  const { play: playLightningStrike } = useSfx(`${BASE}sfx/dragon-studio-lightning-strike-386161.mp3`);
  const { play: playThunderStrike }   = useSfx(`${BASE}sfx/tanweraman-thunder-strike-wav-321628.mp3`);
  const { play: playMusic }           = useMusic(`${BASE}music/juliush-awakening-chill-out-music-1295.mp3`, { loop: true });
  const { play: playBirds }           = useSfx(`${BASE}sfx/loswin23-morning-birds-499429.mp3`);
  return (
    <SceneContent
      textures={textures}
  ...
  return (
    <button
      ...
      onMouseLeave={(e) => {
        ...
        e.currentTarget.style.textShadow = "0 0 18px rgba(212, 148, 10, 0.35)";
      }}
    >
      {label}
    </button>
